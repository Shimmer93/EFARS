...
'''
    ...
    pos3d: V x 3
    hmap: V x w x h
    V: Number of joints, w, h: Downsampled width and height of images
'''

def get_project_matrix(R, t, cali):
    '''
        Compute the project matrix from rotation, translation and calibration
    '''
...
def pos2d_to_hmap(pos2d, img_size, downsample, sigma):
    ...
    hmap /= hmap.max() # Ensure the scale is [0, 1]
    hmap[hmap < 0.001] = 0
    hmap = cv.resize(hmap, (img_size[0]//downsample, img_size[1]//downsample), cv.INTER_AREA)
...
def hmap_to_pos2d(hmap, img_size):
    ...
    for i in range(len(hmap)):
        ...
        pos = np.unravel_index(np.argmax(resized_hmap, axis=None), resized_hmap.shape) # Get the 2D index of the predicted position
        x = int(pos[1])
        y = int(pos[0])
        pos2d.append([x, y])
